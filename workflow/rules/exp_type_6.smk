...
if exp_type == 6:
    ...
    for k in k_values:
        ...
        for read_type in read_types:
            if not os.path.isdir(f"filelists/k_{k}/{read_type}"):
                os.makedirs(f"filelists/k_{k}/{read_type}")
            intersect_files = []
            pivot_files = []
            ...
            with open(f"filelists/k_{k}/{read_type}/pivots_filelist.txt", "w") as fd:
                for f in pivot_files:
                    fd.write(base_dir+f+"\n")
            with open(f"filelists/k_{k}/{read_type}/intersections_filelist.txt", "w") as fd:
