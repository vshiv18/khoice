...
def find_ftp_paths(requested_species, using_file_input, use_refseq, use_genbank, database_name):
    ...
    if use_refseq:
        ...
        command = ("esearch -db assembly -query '\"{}\"[Organism] AND ((latest[filter] OR \"latest genbank\"[filter]) AND all[filter] NOT anomalous[filter])'"
               " | esummary | "
               " xtract -pattern DocumentSummary -block FtpPath -match '@type:GenBank' -element FtpPath " 
               " | sed 's/$/\//' ")
    ...
    if output.isdigit() and int(output) > 0 and not using_file_input:
        response = input(f"\nWe found {output} complete genomes for {requested_species} from {database_name}. Does that\nsound right, would you like to download them (y/n)? ").upper().strip()
    ...
    for ftp_path in output_lines:
        ...
        genome_paths.append(ftp_path + name + "_protein.faa.gz")
...
def main(args):
    ...
    while True:
        ...
        if args.refseq:
            refseq_paths, refseq_choice = find_ftp_paths(requested_species, using_file_input, True, False, "RefSeq")
        if args.genbank:
            genbank_paths, genbank_choice = find_ftp_paths(requested_species, using_file_input, False, True, "GenBank")
        
        # Execute the download of those genomes ...
        num_downloaded = [0, 0]
        if args.refseq and refseq_choice == "yes":
            num_downloaded[0] = download_genomes(refseq_paths, args.output_dir, dataset_num, args.num_genomes, 
                                              args.ratio_genomes, args.decompress)
            downloaded_genomes = True
        if args.genbank and genbank_choice == "yes":
            num_downloaded[1] = download_genomes(genbank_paths, args.output_dir, dataset_num, args.num_genomes, 
                                              args.ratio_genomes, args.decompress)                                      
            downloaded_genomes = True

        if refseq_choice == "yes" or genbank_choice == "yes":
            dataset_summaries.append([dataset_num, requested_species, sum(num_downloaded)])
            dataset_num += 1
            print()
