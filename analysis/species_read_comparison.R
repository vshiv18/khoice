...
make_recall_ribbon_sd <- function(kmer_df, dataset_name, mem_df){
  ...
  plot <- ggplot(kmer_df, aes(color = read_type)) +
    geom_ribbon(aes(x=k, ymin=minus_sd, ymax=plus_sd, fill = read_type),
                alpha=0.2, colour = NA) +
    geom_line(aes(x = k, y = avg, color = read_type, linetype = "Kmer")) +
    geom_hline(aes(yintercept=avg, linetype="Half-MEM", color=read_type), data = hm) +
    geom_hline(aes(yintercept=avg, linetype="MEM", color=read_type), data = m) +
    theme_bw() +
